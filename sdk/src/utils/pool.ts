...
import { QuotePriceUpdate, QuoteTokenMint, WOOFI_TOKENS } from "./constants";
...
export class PoolUtils {
  ...
  public sol_priceFeed!: String;
  public usdc_priceFeed!: String;
  ...
  public getLatestBlockHash = async () => {
    ...
    if (getCluster() == 'mainnet') {
      console.log('getCluster(): ', getCluster())
    }
    ...
    let blockheight = await this.provider.connection.getBlockHeight('confirmed');
    ...
    while (blockheight <= lastValidBlockHeight) {
      await sleep(1000);
      blockheight = await this.provider.connection.getBlockHeight('confirmed');
      console.log("current blockheight", blockheight);
    }
  }

  public getReturnLog = (confirmedTransaction: any) => {
  ...
  public createWooracle = async (token: WOOFI_TOKENS, tokenMint: anchor.web3.PublicKey, feedAccount: anchor.web3.PublicKey, priceUpdate: anchor.web3.PublicKey) => {
    const [wooconfig] = await anchor.web3.PublicKey.findProgramAddressSync(
      [Buffer.from('wooconfig')],
      this.program.programId
    );
  ...
  public checkPool = async (tokenMint: anchor.web3.PublicKey, quoteTokenMint: anchor.web3.PublicKey, feedAccount: anchor.web3.PublicKey, priceUpdate: anchor.web3.PublicKey): Promise<{woopoolData: any, poolVaultBalance: any}> => {
    const [wooconfig] = await anchor.web3.PublicKey.findProgramAddressSync(
      [Buffer.from('wooconfig')],
      this.program.programId
    );
    ...
    try {
      woopoolData = await this.program.account.wooPool.fetch(woopool);
        ...
        console.log(e);
        return {woopoolData: undefined, poolVaultBalance: undefined};
    }
