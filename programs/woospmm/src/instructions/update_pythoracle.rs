...
pub fn handler(ctx: Context<UpdatePythOracle>) -> Result<()> {    
    return update(&mut ctx.accounts.price_update, &mut ctx.accounts.pythoracle);
}

pub fn update<'info>(price_update: &mut Account<'info, PriceUpdateV2>, oracle: &mut Account<'info, Oracle>) -> Result<()> {
    ...
    let price = price_update.get_price_no_older_than(&Clock::get()?, maximum_age, &oracle.feed_account.key().to_bytes())?;
    // Sample output:
